...
cst816t::cst816t(int8_t _rst, int8_t _irq)
{
    rst = _rst;
    irq = _irq;
    chip_id = 0;
...
{
    uint8_t dta[6];
    if ((tp_event || touching) && !i2c_read(CST816T_ADDRESS, REG_GESTURE_ID, dta, sizeof(dta)))
    {
...
uint8_t cst816t::i2c_read(uint16_t addr, uint8_t reg_addr, uint8_t *reg_data, uint32_t length)
{
    ...
    Wire1.requestFrom((uint8_t)addr, (size_t)length, (bool)true);
